function bz_plotPlaceFields(varargin)
...
conditions = length(firingMaps.rateMaps{1});
...
if conditions > tr_conditions
    ...
    spkTrain_length = length(spkTrain);
    spkTrain{spkTrain_length+1} = spkTrain{spkTrain_length};
    spkTrain{spkTrain_length} = spkTrain{spkTrain_length-1};
    
    subSessions_size = size(tracking.events.subSessions);
    tracking.events.subSessions(subSessions_size(1)+1,1) = tracking.events.subSessions(subSessions_size(1),1);
    tracking.events.subSessions(subSessions_size(1)+1,2) = tracking.events.subSessions(subSessions_size(1),2);
    
    tracking.events.subSessions(subSessions_size(1),1) = tracking.events.subSessions(subSessions_size(1)-1,1);
    tracking.events.subSessions(subSessions_size(1),2) = tracking.events.subSessions(subSessions_size(1)-1,2);
    
    subSessionsMask_max = max(tracking.events.subSessionsMask);
    tracking.events.subSessionsMask(tracking.events.subSessionsMask == subSessionsMask_max) = subSessionsMask_max + 1;
    
    tracking.folders{tr_conditions+1} = tracking.folders{tr_conditions};
    tracking.folders{tr_conditions} = tracking.folders{tr_conditions-1};
end
