function [GMI] = bz_GMI_v3(lfp,phaserange,amprange,varargin)
...
if ismember(foldername,tracking.folders)
    ...
    for jj = 1:max(vvv(:))
        ...
        if ttt > MINPFSIZE
            ...
            if RatTD > thres
                ...
                hist(end)=[];
                [pval,z]=circ_rtest((edges(1:end-1)+edges(2:end))/2,hist);  %% 
    ...
    binde = 0:intSize:360;
    figure,
    subplot(2,1,1); errorbar(binde,MeanPowGa,MeanPowStd,'LineWidth',2),
    title(['Gamma Power/Theta Modulation ',' GMI  ',num2str(GMI)]);
