function [comod ] = bz_PhaseAmpCouplingByAmp_NP(lfp,sig1band,sig2band,varargin)
...
if ~exist_file
    ...
    if makePlot
        ...
        if saveFig
            if ~isempty(foldername)
               saveas(gcf,['lfpAnalysisFigures\PhaseAmpCouplingByAmp.',foldername,'_',num2str(sig2band(1)),num2str(sig2band(end)),'_',num2str(times(1)),'-',num2str(times(end)),'.png'])
            else
