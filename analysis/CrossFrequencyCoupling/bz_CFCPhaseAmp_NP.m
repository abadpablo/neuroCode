function [comodulogram] = bz_CFCPhaseAmp_NP(lfp,phaserange,amprange,varargin)
...
if ~exist_file
    ...
    if makePlot  
        ...
        if saveFig
            if ~isempty(foldername)
                saveas(gcf,['lfpAnalysisFigures\CFCPhaseAmp.',foldername,'_',num2str(amprange(1)),num2str(amprange(end)),'_',num2str(times(1)),'-',num2str(times(end)),'Hz.png'])
            else
