...
% Let's create a part in the script that takes into account that we will
% only analyze linear track and we will neglect open field
behaviour = getSessionBehaviour_v2();

% Find place fields
for unit = 1:length(firingMaps.rateMaps)     
    for c = 1:length(firingMaps.rateMaps{1})
        if size(behaviour.maps{c},2) == 2 % only linear track
            % Default values
            ...
            x = 1:length( firingMaps.rateMaps{1}{c});
