function [PhaseLockingData] = bz_PhaseModulationAbad(varargin)
% USAGE
%   [PhaseLockingData] = bz_PhaseModulationAbad(varargin)  
%
% INPUTS
...
%                   phase modulation. Formats accepted: tstoolbox intervalSet or a 2column
%                   matrix of [starts stops] in seconds
%   samplingRate - specifies lfp sampling frequency. default = 1250
%   method       - method selection for how to generate phase,
%                  possibilities are: 'hilbert' (default) or 'wavelet'
%   powerThresh  - integer power threshold to use as cut off, measured in
%                   standard deviations (default = 2)
%   plotting     - logical if you want to plot, false if not, default =
%                   true
...
%
% Pablo Abad 2021, based on bz_PhaseModulation by Brendon Watson
% This scrip takes only the lfo for subsessions and then I need to
% substract the times of the unit in order to be coherent with the LFP
% timestamps
...
for a = 1:length(spikes.times)
    ...
    [bools,interval,index] = InIntervals(spikes.times{a},intervals);
...
detectorName = 'bz_PhaseModulationAbad';
