function [coherogram] = bz_MTCoherogram_NP(lfp1,lfp2,varargin)
...
if strcmpi(show,'on')
    ...
    if saveFig
      if ~isempty(foldername)
        saveas(gcf,['lfpAnalysisFigures\Power_WholeRecording_',foldername,'_',num2str(times(1)),'-',num2str(times(end)),'.png']);
      else    
    ...
    if ismember(foldername,tracking.folders)
        ...
        ts = InIntervals(tracking.timestamps,times);
        subplot(5,1,5);plot(tracking.timestamps(ts),tracking.position.speed(ts)),title('Speed vs time'),colorbar,axis([times(1) times(end) 0  max(tracking.position.speed(ts))+0.01]); 
    end
    
    if saveFig
      if ~isempty(foldername)
        saveas(gcf,['lfpAnalysisFigures\Periodogram_WholeRecording_',foldername,'_',num2str(times(1)),'-',num2str(times(end)),'.png']);
      else    
...
if filtering
    ...
    if ismember(foldername,tracking.folders)
        ...
        MINPFSIZE = tracking.samplingRate(1) * 2;
        [n2,bin2] = histc(tracking.timestamps(ts),t);
        ...
        if exist('coherency_epochs','var')
            ...
            if strcmpi(show,'on')
                ...
                if saveFig
                    if ~isempty(foldername)
                        saveas(gcf,['lfpAnalysisFigures\Power_',foldername,'_',num2str(times(1)),'-',num2str(times(end)),'_filtered','.png']);
                    else    
                ...
                subplot(5,1,5);plot(tracking.timestamps(ts),v),title('Speed vs time'),colorbar, axis([0 max(tracking.timestamps(ts)) 0  max(v)+0.01]); hold on;
                subplot(5,1,5); plot(tracking.timestamps(cb_spd),v(cb_spd),'r'), title('Speed vs time epoch'),colorbar, axis([times(1) times(end) 0  max(v)+0.01]);

                if saveFig
                    if ~isempty(foldername)
                        saveas(gcf,['lfpAnalysisFigures\Periodogram_',foldername,'_',num2str(times(1)),'-',num2str(times(end)),'_filtered','.png']);
                    else    
